...
bool NetlinkManager::onFdReadable(int fd)
{
    ...
    for(hdr = (struct nlmsghdr*)buf; NLMSG_OK(hdr, ret); hdr = NLMSG_NEXT(hdr, ret))
    {
        ...
        switch(hdr->nlmsg_type)
        {
            ...
            case RTM_DELADDR:
                ...
                break;
            default:
                LOG_WARN("Unknown netlink nlmsg_type %d\n", hdr->nlmsg_type);
                break;
        }
    }
...
void NetlinkManager::parseLinkMsg(struct nlmsghdr *h)
{
    ...
    if(!tb[IFLA_IFNAME])
    {
        LOG_ERROR("IF_NAME is NULL \n");
        return;
    }

    strncpy(ifname, (char*)RTA_DATA(tb[IFLA_IFNAME]), IFNAMSIZ-1);
...
#if 0
    if(tb[IFA_LOCAL])
        LOG_DEBUG("   IFA_LOCAL     %s\n", inet_ntop(ifa->ifa_family, RTA_DATA(tb[IFA_LOCAL]), ifaddr, 256));
    if(tb[IFA_ADDRESS])
        LOG_DEBUG("   IFA_ADDRESS   %s\n", inet_ntop(ifa->ifa_family, RTA_DATA(tb[IFA_ADDRESS]), ifaddr, 256));
    if(tb[IFA_BROADCAST])
        LOG_DEBUG("   IFA_BROADCAST %s\n", inet_ntop(ifa->ifa_family, RTA_DATA(tb[IFA_BROADCAST]), ifaddr, 256));
#endif
